  ...
  def start_quiz_new_round(round_multiplier = 1)
    ...
    if !correct_answers[0] || !correct_answers[1]
      ...
      if !correct_answers[0] #1st question redemption
        new_question(calculate_points_awarded(base_points_q1_redemption, round_multiplier), _INTL("Which Pokémon is this fusion's body?"), @body_id, true, false)
      ...
      if !correct_answers[1] #2nd question redemption
        new_question(calculate_points_awarded(base_points_q2_redemption, round_multiplier), _INTL("Which Pokémon is this fusion's head?"), @head_id, true, false)
      end
    else
