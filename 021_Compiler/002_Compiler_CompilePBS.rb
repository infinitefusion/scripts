module Compiler
  ...
  def compile_pokemon(path = "PBS/pokemon.txt")
    ...
    all_evos = {}
    GameData::Species.each do |species|
      # Build a hash of prevolutions for each species
      species.evolutions.each do |evo|
        all_evos[evo[0]] = [species.species, evo[1], evo[2], true] if !all_evos[evo[0]]
      end
    end
    GameData::Species.each do |species|
      # Distribute prevolutions
  ...
  def compile_trainers(path = "PBS/trainers.txt")
    ...
    trainer_preRematchText_evolved = []
    ...
    pbCompilerEachPreppedLine(path) { |line, line_no|
      if line[/^\s*\[\s*(.+)\s*\]\s*$/]
        ...
        case property_name
        when "Items", "LoseText", "BattleText", "LoseTextRematch", "PreRematchText", "PreRematchText_caught", "PreRematchText_evolved", "PreRematchText_fused", "PreRematchText_unfused", "PreRematchText_reversed", "PreRematchText_gift", "TrainerInfo"
          ...
          trainer_preRematchText_evolved[trainer_id] = property_value if property_name == "PreRematchText_evolved"
          ...
          trainer_infoText[trainer_id] = property_value if property_name == "TrainerInfo"
