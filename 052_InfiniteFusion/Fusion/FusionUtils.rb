...
def get_triple_fusion_path(dexNum)
    case dexNum
...
def get_triple_fusion_components(species_id)
  dex_num = species_id.is_a?(GameData::Species) ? GameData::Species.get(species_id).id_number : species_id
  return get_triple_fusion_path(dex_num).split(".").map(&:to_i)
