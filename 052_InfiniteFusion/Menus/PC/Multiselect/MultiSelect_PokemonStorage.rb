class PokemonStorage
  ...
  def pbStoreCaughtBatch(pokemon_positions_array, box_index = @currentBox, x = 0, y = 0)
    ...
    pokemon_positions_array.each_with_index do |pokemon_data_array, i|
      ...
      intended_position = [x + relative_position[0],y + relative_position[1]]
      if intended_position[0] > box_width || intended_position[1] > box_height
        intended_position = nil
      end
      if intended_position
        index = intended_position[1] * box_width + intended_position[0]
        if self[box_index, index].nil?
          assignments[i] = intended_position   # Lock this Pokémon to its free slot
        else
          unplaced << i               # Slot already taken, will assign later
        end
      else
        unplaced << i                 # No explicit intended slot
      end
    end

    # -----------------------------
    # Phase 2: Assign leftovers by distance
    # -----------------------------
